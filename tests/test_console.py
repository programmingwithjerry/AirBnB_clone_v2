...
class TestHBNBCommand(unittest.TestCase):
    ...
    def test_fs_create(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            cons = HBNBCommand()
            cons.onecmd('create City name="Abuja"')
            mdl_id = cout.getvalue().strip()
            clear_stream(cout)
            self.assertIn('City.{}'.format(mdl_id), storage.all().keys())
            cons.onecmd('show City {}'.format(mdl_id))
            self.assertIn("'name': 'Abuja'", cout.getvalue().strip())
            clear_stream(cout)
            cons.onecmd('create User name="Ojonuba" age=20 height=7.7')
            mdl_id = cout.getvalue().strip()
            self.assertIn('User.{}'.format(mdl_id), storage.all().keys())
            clear_stream(cout)
            cons.onecmd('show User {}'.format(mdl_id))
            self.assertIn("'name': 'Ojonuba'", cout.getvalue().strip())
            self.assertIn("'age': 20", cout.getvalue().strip())
            self.assertIn("'height': 7.7", cout.getvalue().strip())
    ...
    def test_db_create(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            clear_stream(cout)
            cons.onecmd('create User email="mrjerry1@gmail.com" password="1249"')
            mdl_id = cout.getvalue().strip()
            dbc = MySQLdb.connect(
    ...
    def test_db_show(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            obj = User(email="mrjerry@gmail.com", password="1249")
            dbc = MySQLdb.connect(
                host=os.getenv('HBNB_MYSQL_HOST'),
                port=3306,
                user=os.getenv('HBNB_MYSQL_USER'),
            ...
            result = cursor.fetchone()
            self.assertTrue(result is not None)
            self.assertIn('mrjerry@gmail.com', result)
            self.assertIn('1249', result)
            self.assertIn('mrjerry@gmail.com', cout.getvalue())
            self.assertIn('1249', cout.getvalue())
            cursor.close()
            dbc.close()
